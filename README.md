...
Скрипт для вывода статистики WireGuard и создания новых пиров. В репозитории
также есть Telegram-бот для удалённого просмотра статистики.
...
## Телеграм-бот
Бот использует `wgstat.sh` и работает только с разрешения администратора.

### Конфигурация
- `BOT_TOKEN` — токен Telegram-бота.
- `BOT_ADMIN_ID` — числовой Telegram ID администратора. Только этот пользователь
  сможет выполнять `/stats`.
- `WGSTAT_CMD` — путь к `wgstat.sh` (по умолчанию `/usr/local/sbin/wgstat.sh`).

### Команды
- `/stats [peer]` — показать статистику по всем или конкретному пиру (доступно
  только администратору).
- `/id` — отправить свой Telegram ID.
- `/start` — краткая справка о боте.
- `/help` — список команд.
